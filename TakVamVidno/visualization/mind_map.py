...
class MindMap:
    ...
    def visualize(self, main_topic: str, subtopics: dict[str, list[str]]) -> bytes:
        ...
        Детали подтемы должны быть уникальны.
        ...
        Known Issues
        ------------
        - Иногда igraph решает поставить root-node не основную идеи, а подтему, что немного портит внешний вид и вредит усваиванию информации

        TODO
        ----
        - Кастомизацию, позволить менять ориентацию куда идёт график, цвета блоков, итд.
        - Сделать возможность продлевать это древо, щас максимальная глубина это 3 (основная тема -> подтема -> детали), надо сделать это динамическим.
        """
